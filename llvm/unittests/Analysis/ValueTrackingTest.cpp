...
TEST_F(ComputeKnownFPClassTest, FMul) {
  parseAssembly("define float @test(float nofpclass(nan inf) %nnan.ninf0, "
                "float nofpclass(nan inf) %nnan.ninf1, float nofpclass(nan) "
                "%nnan, float nofpclass(qnan) %no.qnan, float %unknown) {\n"
...
TEST_F(ComputeKnownFPClassTest, FMulNoZero) {
  parseAssembly(
      "define float @test(float nofpclass(zero) %no.zero, float nofpclass(zero "
      "nan) %no.zero.nan0, float nofpclass(zero nan) %no.zero.nan1, float "
      "nofpclass(nzero nan) %no.negzero.nan, float nofpclass(pzero nan) "
      "%no.poszero.nan, float nofpclass(inf nan) %no.inf.nan, float "
      "nofpclass(inf) %no.inf, float nofpclass(nan) %no.nan) {\n"
...
TEST_F(ValueTrackingTest, ComputeConstantRange) {
  ...
  define i32 @test(i32 %x.1, i32 %x.2) {
    ...
    ConstantRange CR3 =
        computeConstantRange(X2, false, true, &AC, I, nullptr, 6);
